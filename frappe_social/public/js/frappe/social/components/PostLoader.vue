<template>
  <div>
    ...
    <div
      v-if="!loading_posts && !posts.length"
      class="no-post-message text-muted"
    >
      {{ __("No posts yet") }}
    </div>
    <div
      v-else-if="loading_posts && posts.length"
...
export default {
  ...
  methods: {
    ...
    update_posts(load_old = false) {
      ...
      this.get_posts(filters, load_old)
        .then((res) => {
          if (load_old) {
            if (!res.length) {
