...
export default {
	...
	data() {
		return {
			...
			is_globally_pinnable:
				frappe.user_roles.includes("System Manager") && frappe.social.is_home_page(),
			is_pinnable: false,
	...
	computed: {
		...
		is_pinned() {
			return (
				(false && frappe.social.is_profile_page(this.post.owner)) ||
	...
	methods: {
		...
		toggle_pin() {
			if (this.is_globally_pinnable) {
				frappe.db
					.set_value("Post", this.post.name, "is_globally_pinned", cint(!this.is_pinned))
					.then(
						(res) =>
							(this.post.is_globally_pinned = cint(res.message.is_globally_pinned))
					);
			}
			if (this.is_pinnable) {
				frappe.db
					.set_value("Post", this.post.name, "is_pinned", cint(!this.is_pinned))
		...
		generate_preview(link_element) {
			...
			frappe
				...
				.then((info) => {
					...
					const description = frappe.ellipsis(
					...
					if (title) {
						link_element.insertAdjacentHTML(
							"afterend",
							`
